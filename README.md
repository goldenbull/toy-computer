...
最重要的任务，是如何以符合认知规律的方式，从已知到未知，以问题为导向，逐渐引入新概念，最终完成整个拼图

其中最难的环节在函数部分，函数涉及到很多机制，在完成“惊险一跃”之前，很难看清这些机制的作用，需要研究如何进行合理的问题导向

## 入门

- 上手熟悉：mov add print指令
- 加减乘除指令，input指令，一个计算器，输入两个数，输出加减乘除的结果
- 从1累加到10，逐行显示 `n sum(1..=n)`
...
- 显然，稍微复杂一点点的计算，4个寄存器就不够用了，引入内存的概念
...
张益唐的故事，引入孪生质数概念，另一个更直接的例子是哥德巴赫猜想
