...
| pause | `pause`                           | 暂停，回车后继续运行，主要为了方便观察dump的结果                     | `pause`                           |
...
| 指令    | <div style="width:120px">格式</div> | 含义                                                                |
|-------|-----------------------------------|-------------------------------------------------------------------|
| push  | `push N`                          | 等价于<br/>`mov [sp], N`<br/>`sub sp, 1`                             |  
|       | `push r1`                         | 等价于<br/>`mov [sp], r1`<br/>`sub sp, 1`<br/>r1只能是通用寄存器 ax bx cx dx |
| pop   | `pop r1`                          | 等价于<br/>`add sp, 1`<br/>`mov r1, [sp]`                            |
|       | `pop`                             | 等价于<br/>`add sp, 1`<br/>                                          |
...
为此我们引入函数调用最核心的两条指令：call和ret指令

| 指令   | <div style="width:120px">格式</div> | 含义                       | 说明                                         |
|------|-----------------------------------|--------------------------|--------------------------------------------|
| call | `call label`                      | 将`ip`寄存器入栈，同时跳转到`label`处 | 可以理解为 `push ip+1 同时 jmp label`，两条指令合并成一条指令 |
| ret  | `ret`                             | 可以理解为`pop ip`            | 栈中的`ip`是函数返回后，下一条要执行的指令                    |
...
```
------------------
b       第二个参数   <-- 开始一次函数调用前，sp 指向这个位置
a       第一个参数
r       函数的返回值
ip      调用 call 时 ip 压入栈
bp      前一个 frame 的起点
x1      函数内部运算时需要的临时变量  <-- 执行函数内部的指令时，bp 指向这个位置
x2      第二个临时变量
...
2. 使用递归函数实现斐波那契数列的计算，并使用 `--step` 模式跟踪观察递归函数的调用和返回的过程
...
使用 `-s` 或者 `--step` 参数可以进入逐行模式，每执行一条指令，就会自动执行一次 `dump`

## TODO

使用web界面，实现更好的可视化和交互功能，更好的方便教学使用