...
3. 生成10个随机数并显示，然后输入两个编号选择其中的两个数，输出这两个数的加减乘除的各种结果
...
需要完成的任务：
1. 从1加到10，输出结果
2. 生成100个随机数并显示，然后输入两个编号选择其中的两个数，输出这两个数的加减乘除的各种结果
3. 输出斐波那契数列的前100项
...
9. 输出100以内的所有质数，要求：
   ...
   3.  在调用之前，把寄存器的数保存到内存特定位置，等从8的代码回来之后，再从内存中恢复之前保存的寄存器的值
...
通过dump指令理解jmp指令是如何将label转换为数字的
...
将一段代码变成可以重复调用的函数，会有一堆问题：
1. 如何调回去：jmp之前，想办法保存当前指令的序号，函数执行完毕后，jmp回来继续执行即可
2. 调用者并不确定函数会用到哪些寄存器，所以当前指令的序号只能保存在内存里，且必须是事先约定的内存位置（实际上这一片内存称为stack，即“栈”）
