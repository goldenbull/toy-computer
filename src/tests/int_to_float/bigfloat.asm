...
;   [bx+2] = PRECISION = 16
;
; BigFloat structure (16 cells, fits one memory row):
;   [ax+0]  = sign (1 or -1)      (where ax = base address)
;   [ax+1]  = exponent
;   [ax+2]  = values[0] (most significant)
;   [ax+3]  = values[1]
;   ...
;   [ax+15] = values[13] (least significant)
...
_calc_base_done:
    ...
    ; PRECISION = 14 (so BigFloat = 2 + 14 = 16 cells, fits one memory row)
    mov ax, 14
    mov [bx+2], ax
...
;   [bp-3] = local: temp[1]
;   ...
;   [bp-29] = local: temp[27]  (2*PRECISION-1 = 27)
;
; Caller convention:
;   push result_addr
;   push a_addr
...
_mul_bigfloat:
    push bp
    mov bp, sp
    sub sp, 30       ; 2 locals (i, j) + 28 temp array
...
;
; Note: Uses addresses 592, 608, 624 as temp storage
;
; Caller convention:
;   push result_addr
;   push a_addr
...
_mod_bigfloat:
    ...
    mov ax, 592
    mov [bp+0], ax   ; temp1 = 592 (quotient)
    mov ax, 608
    mov [bp-1], ax   ; temp2 = 608 (truncated quotient)
    mov ax, 624
    mov [bp-2], ax   ; temp3 = 624 (quotient * b)
...
main:
    ...
    ; ========== Simple test: divide 22 by 3 repeatedly ==========
    println "--- Divide 22 by 3, 10 times ---"
